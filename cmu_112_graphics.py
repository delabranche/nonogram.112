# cmu_112_graphics.py with added rightMousePressed/Released/Dragged
# version 0.9.0
...
class App(object):
    ...
    def rightMousePressed(app, event): pass # use event.x and event.y
    def rightMouseReleased(app, event): pass # use event.x and event.y
    def rightMouseDragged(app, event): pass #use event.x and event.y
    ...
    def _rightMousePressedWrapper(app,event):
    	...
    	if ((event.x < 0) or (event.x > app.width) or
            ...
            app._rightMouseIsPressed = True
            app._rightLastMousePosn = (event.x, event.y)
            if (app._methodIsOverridden('rightMousePressed')):
                app.rightMousePressed(event)
                app._redrawAllWrapper()
    @_safeMethod
    def _rightMouseReleasedWrapper(app, event):
        if (not app._running) or app._paused: return
        app._rightMouseIsPressed = False
        if app._rightMousePressedOutsideWindow:
            app._rightMousePressedOutsideWindow = False
            app._sizeChangedWrapper()
        else:
            app._rightLastMousePosn = (event.x, event.y)
            if (app._methodIsOverridden('rightMouseReleased')):
                app.rightMouseReleased(event)
                app._redrawAllWrapper()
    ...
    def _mouseMotionWrapper(app):
        ...
        rightMouseDraggedExists = app._methodIsOverridden('rightMouseDragged')
        if ((not app._paused) and
            (not app._mousePressedOutsideWindow) and
            (((not (app._mouseIsPressed or app._rightMouseIsPressed)) and mouseMovedExists) or
        ...
        if((not app._paused) and (not app._rightMousePressedOutsideWindow) and
        		(app._rightMouseIsPressed and rightMouseDraggedExists)):
        	class MouseMotionEvent(object): pass
        	event = MouseMotionEvent()
        	root = app._root
        	event.x = root.winfo_pointerx() - root.winfo_rootx()
        	...
        	if ((app._lastMousePosn !=  (event.x, event.y)) and
                (event.x >= 0) and (event.x <= app.width) and
                (event.y >= 0) and (event.y <= app.height)):
        		if (app._rightMouseIsPressed):
        			app.rightMouseDragged(event)
        		app._lastMousePosn = (event.x, event.y)
        		app._redrawAllWrapper()
        if (mouseMovedExists or mouseDraggedExists or rightMouseDraggedExists):
    ...
    def run(app):
        ...
        if (App._theRoot is None):
            ...
            App._theRoot.bind("<B3-ButtonRelease>", lambda event: App._theRoot.app._rightMouseReleasedWrapper(event))
...
class TopLevelApp(App):
    ...
    def rightMouseReleased(app, event): app._callFn("rightMouseReleased", app, event)
    def mouseMoved(app, event): app._callFn('mouseMoved', app, event)
    def mouseDragged(app, event): app._callFn('mouseDragged', app, event)
    def rightMouseDragged(app, event): app._callFn("rightMouseDragged", app, event)
